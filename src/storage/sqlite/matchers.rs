//! Label matcher query building for SQLite storage.
//!
...
//!
//! SQLite REGEXP support is enabled via sqlx's `regexp` feature and `.with_regexp()`
//! which registers a regexp() user function using Rust's regex crate.
...
impl SqliteStorage {
    ...
    pub(super) async fn find_sensors_by_matchers(
        ...
        for matcher in name_matchers {
            let clause = match matcher.matcher_type {
                ...
                MatcherType::RegexMatch => {
                    params.push(matcher.value.clone());
                    let clause = format!("s.name REGEXP ?{}", param_idx);
                    param_idx += 1;
                    clause
                }
                MatcherType::RegexNotMatch => {
                    params.push(matcher.value.clone());
                    let clause = format!("NOT (s.name REGEXP ?{})", param_idx);
                    param_idx += 1;
                    clause
                }
            };
                ...
                MatcherType::RegexMatch => {
                    ...
                    let subquery = format!(
                        r#"s.sensor_id IN (
                            ...
                            JOIN labels_name_dictionary lnd ON l.name = lnd.id
                            JOIN labels_description_dictionary ldd ON l.description = ldd.id
                            WHERE lnd.name = ?{} AND ldd.description REGEXP ?{}
                        )"#,
                        param_idx,
                        param_idx + 1
                    );
        ...
        // Build dynamic IN clause for sensor IDs
        let placeholders: Vec<String> = (1..=sensor_ids.len()).map(|i| format!("?{}", i)).collect();
        ...
        let labels_rows = labels_query.fetch_all(&self.pool).await?;
        ...
        for row in sensor_rows {
            let sensor_uuid = Uuid::parse_str(&row.uuid).map_err(|e| {
                anyhow::Error::from(StorageError::invalid_data_format(
                    &format!("Failed to parse sensor UUID '{}': {}", row.uuid, e),
                    None,
                    Some(&row.name),
                ))
            })?;
