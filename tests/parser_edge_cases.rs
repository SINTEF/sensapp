...
use sensapp::infer::columns::{infer_column, InferedColumn};
use sensapp::infer::parsing::{infer_type, infer_type_with_trim, InferedValue};
...
mod parsing_edge_cases {
    ...
    fn test_empty_strings_and_whitespace() {
        ...
        assert_eq!(infer_type(""), Ok(("", InferedValue::String("".to_string()))));
        ...
        assert_eq!(infer_type_with_trim("  42  "), Ok(("", InferedValue::Integer(42))));
        assert_eq!(infer_type_with_trim("\t3.15\n"), Ok(("", InferedValue::Float(3.15))));
    }

    #[test]
    fn test_special_numeric_values() {
        // Very large integers
        assert_eq!(infer_type("9223372036854775807"), Ok(("", InferedValue::Integer(9223372036854775807))));
        
        // Very small integers
        assert_eq!(infer_type("-9223372036854775808"), Ok(("", InferedValue::Integer(-9223372036854775808))));
        ...
        assert_eq!(infer_type("inf"), Ok(("", InferedValue::String("inf".to_string()))));
        assert_eq!(infer_type("NaN"), Ok(("", InferedValue::String("NaN".to_string()))));
    }
    ...
    fn test_boolean_variations() {
        ...
        assert_eq!(infer_type("yes"), Ok(("", InferedValue::String("yes".to_string()))));
        assert_eq!(infer_type("no"), Ok(("", InferedValue::String("no".to_string()))));
    ...
    fn test_datetime_edge_cases() {
        ...
        let datetime2 = sensapp::infer::parsing::parse_iso8601_datetime("2024-12-31T23:59:59+01:00");
        ...
        assert_eq!(infer_type("01/01/2024"), Ok(("", InferedValue::String("01/01/2024".to_string()))));
        assert_eq!(infer_type("2024-01-01"), Ok(("", InferedValue::String("2024-01-01".to_string()))));
    }
    ...
    fn test_json_edge_cases() {
        ...
        assert!(matches!(infer_type("[]"), Ok((_, InferedValue::Json(_)))));
        assert!(matches!(infer_type("{\"key\": \"value\"}"), Ok((_, InferedValue::Json(_)))));
        assert!(matches!(infer_type("[1, 2, 3]"), Ok((_, InferedValue::Json(_)))));
        
        // Invalid JSON (should be strings)
        assert_eq!(infer_type("{key: value}"), Ok(("", InferedValue::String("{key: value}".to_string()))));
        assert_eq!(infer_type("{\"key\":}"), Ok(("", InferedValue::String("{\"key\":}".to_string()))));
        assert_eq!(infer_type("[1,2,3,]"), Ok(("", InferedValue::String("[1,2,3,]".to_string()))));
    }

    #[test]
    fn test_unicode_and_special_characters() {
        // Unicode strings
        assert_eq!(infer_type("café"), Ok(("", InferedValue::String("café".to_string()))));
        assert_eq!(infer_type("北京"), Ok(("", InferedValue::String("北京".to_string()))));
        assert_eq!(infer_type("🚀"), Ok(("", InferedValue::String("🚀".to_string()))));
        
        // Special characters that might interfere with parsing
        assert_eq!(infer_type("data,with,commas"), Ok(("", InferedValue::String("data,with,commas".to_string()))));
        assert_eq!(infer_type("data\nwith\nnewlines"), Ok(("", InferedValue::String("data\nwith\nnewlines".to_string()))));
        assert_eq!(infer_type("data\"with\"quotes"), Ok(("", InferedValue::String("data\"with\"quotes".to_string()))));
    }

    #[test]
    fn test_mixed_type_precedence() {
        // When mixing types, string should win
        let mixed_column = vec![
            "42".to_string(),
            "hello".to_string(),
            "3.14".to_string(),
        ];
...
mod integration_tests {
    ...
    fn test_complex_mixed_data_inference() {
        ...
        let columns = vec![
            ...
            // Sensor IDs (strings)
            InferedColumn::String(vec!["TEMP_001".to_string(), "TEMP_002".to_string(), "TEMP_003".to_string()]),
            // GPS coordinates
            InferedColumn::Float(vec![59.9139, 59.9140, 59.9141]),
