...
class WsProtocol extends AbstractProtocol{
  def send(root: Root, subscription: Option[Subscription], sensor: String) {
    ...
    subscription.get.hooks.par foreach { url =>
      ...
      //val webSocket = wsServer.createWebsocket(data._1._1, data._1._2)
      /*webSocket.send(Some(root).toString())*/

      Some(root).foreach(r => {
        WsServerFactory.myServer.getClientWebSocket(0).send(JsonParser.toJson(root))
      })
    }
  }
}
